/**
 * Vite Configuration for Coverage-Instrumented Builds
 * 
 * This config extends the main vite.config.ts and adds Istanbul instrumentation
 * for collecting code coverage from E2E tests running in the browser.
 */
...
export default defineConfig({
  plugins: [
    react(),
    // Istanbul instrumentation for coverage collection
    istanbul({
      include: ['src/**/*'],
      exclude: [
        'node_modules/**',
    ...
    }),
    {
      ...
      writeBundle() {
        ...
        console.log(`✓ Manifest and assets copied for ${browser} (coverage build)`);
      },
    },
    {
      ...
      writeBundle() {
        ...
        console.log(`✓ HTML files restructured for ${browser} (coverage build)`);
      },
    },
    {
      // Build content script as IIFE with instrumentation
      ...
      async closeBundle() {
        ...
        await build({
          ...
          build: {
            ...
            minify: false, // Don't minify for coverage
            sourcemap: 'inline',
          },
          define: {
        ...
        console.log(`✓ Content script built as IIFE with coverage instrumentation for ${browser}`);
      },
    },
  ],
  base: './',
